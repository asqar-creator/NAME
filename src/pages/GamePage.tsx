...
export function GamePage({ onHome }: { onHome: () => void }) {
  ...
  useEffect(() => { if (!game.winner) { levelAdvanced.current = false; return; } if (mode === 'bot' && game.winner === 'player' && !levelAdvanced.current) { levelAdvanced.current = true; const next = level + 1; localStorage.setItem('clash-bot-level', String(next)); setLevel(next); setCrystals((value) => { const reward = value + 30; localStorage.setItem('clash-crystals', String(reward)); return reward; }); } }, [game.winner, mode, level]);
  ...
  return <main className="game-shell">
    ...
    {showResult && <div className="result" role="dialog" aria-modal="true"><div className="result__card">
      ...
      <p>{game.winner === 'player' ? mode === 'bot' ? `🎉 Ура! Ты прошёл уровень ${Math.max(1, level - 1)} и получил 💎 30 кристаллов! Следующий уровень ${level} будет сложнее.` : '🎉 Ура! Вражеская крепость пала — ты победил!' : 'Попробуй снова: собери новую армию и защити свою базу.'}</p>
