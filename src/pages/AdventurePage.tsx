...
export function AdventurePage({ onHome }: { onHome: () => void }) {
  ...
  useEffect(() => { const secretLoad = () => { try { const raw = localStorage.getItem(ADVENTURE_SAVE_KEY); if (!raw) return; const data = JSON.parse(raw) as { level?: number; gold?: number; crystalBank?: number; ownedSkins?: string[]; ownedWeapons?: string[]; skin?: string; weapon?: string; lives?: number; difficulty?: Difficulty; player?: { x: number; y: number } }; setLevel(data.level ?? 1); setGold(data.gold ?? 3); setCrystalBank(data.crystalBank ?? 0); setOwnedSkins(data.ownedSkins ?? ['🧑']); setOwnedWeapons(data.ownedWeapons ?? ['⚔️']); setSkin(data.skin ?? '🧑'); setWeapon(data.weapon ?? '⚔️'); setLives(data.lives ?? 5); setDifficulty(data.difficulty ?? 'normal'); if (data.player) setPlayer(data.player); } catch { /* Секретная загрузка не изменяет повреждённое сохранение. */ } }; window.addEventListener('load-all-games', secretLoad); return () => window.removeEventListener('load-all-games', secretLoad); }, []);
  ...
  return <main className="adventure-page">
    ...
    <details className="game-story game-story--dark"><summary>📖 Новая история острова</summary><p>Карта, найденная в бутылке, привела героя к Архипелагу Потерянной Короны. Пираты раскололи корону на кристаллы, а её тёмная сила пробудила морских чудовищ. Собери осколки, найди легендарный трезубец, победи капитанов и закрой врата Демогоргона.</p></details>
