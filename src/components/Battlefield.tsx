...
function AskarHero({ cooldown, evil = false }: { cooldown: number; evil?: boolean }) {
  return <div className={`askar-hero${evil ? ' askar-hero--evil' : ''}`} aria-label={evil ? 'Злой Аскар' : 'Герой Аскар'}>
...
export function Battlefield({ game }: { game: GameState }) {
  return <section className="battlefield" aria-label="Трёхмерное поле боя"><div className="world3d">
    ...
    <AskarHero cooldown={game.heroBanCooldown} />
    <AskarHero cooldown={game.enemyHeroBanCooldown} evil />
