...
function simulate(previous: GameState, dt: number, mode: GameMode, level: number, botUnlockedNames: string[]): GameState {
  ...
  if (mode === 'bot' && game.enemyTimer >= Math.max(.45, 1.25 - (level - 1) * .07)) {
    ...
    const kind = usedSpell ? null : chooseBotMinion(game, botUnlockedNames);
...
export function useBattle(mode: GameMode, paused = false, level = 1, botUnlockedNames: string[] = MINIONS.slice(0, 3).map((kind) => kind.name)) {
  const [game, setGame] = useState(initialGame);
  const botRosterKey = botUnlockedNames.join('|');
  useEffect(() => { if (paused) return; const roster = botRosterKey.split('|').filter(Boolean); const timer = window.setInterval(() => setGame((current) => simulate(current, .1, mode, level, roster)), 100); return () => window.clearInterval(timer); }, [mode, paused, level, botRosterKey]);
